...
function renderHome() {
  ...
  const rows = LANES.map(l => {
    ...
    return `
      <div class="funnel-row" tabindex="0" role="button" title="Filtrer : ${l.label}"
        onclick="goBoard('${l.id}')"
        onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();goBoard('${l.id}')}">
